...
public class Bear extends Animal {
  ...
  public static String name="Bear";
    ...
    int numberOfCubs=1;
    double mass=500;
    double howMuchFood=10;
    double foodMass=0;
    int numberOfAnimalsInCage=5;
    int speed=2;
	...
	result.putIfAbsent(80, BoaConstrictor.name);
	result.putIfAbsent(40, Horse.name);
	result.putIfAbsent(80, Deer.name);
	result.putIfAbsent(80, Rabbit.name);
	result.putIfAbsent(90, Mouse.name);
	result.putIfAbsent(70, Goat.name);
	result.putIfAbsent(70, Sheep.name);
	result.putIfAbsent(50, Hog.name);
	result.putIfAbsent(20, Buffalo.name);
	result.putIfAbsent(10, Duck.name);
	return result;
    }


    public Bear(int numberOfCubsIn,int numberOfStart) {
